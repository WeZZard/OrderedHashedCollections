...
class OrderedDictionaryTests: XCTestCase {
    ...
    func testInitWithArrayLiteral_whichContainsOneElement() {
        let orderedDictionary: OrderedDictionary = [
            (key: "A", value: 1)
        ]
        XCTAssert(orderedDictionary._orderedKeys.count == 1)
        XCTAssert(orderedDictionary._orderedKeys[0] == "A")
        XCTAssert(orderedDictionary._valuesForKeys.count == 1)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[0]] == 1)
    }
    
    func testInitWithArrayLiteral_whichContainsTwoElements() {
        let orderedDictionary: OrderedDictionary = [
            (key: "A", value: 1),
            (key: "B", value: 2),
        ]
        XCTAssert(orderedDictionary._orderedKeys.count == 2)
        XCTAssert(orderedDictionary._orderedKeys[0] == "A")
        XCTAssert(orderedDictionary._orderedKeys[1] == "B")
        XCTAssert(orderedDictionary._valuesForKeys.count == 2)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[0]] == 1)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[1]] == 2)
    }
    
    func testInitWithArrayLiteral_whichContainsTwoElementWithTheSameKeyAndDifferentValue() {
        let orderedDictionary: OrderedDictionary = [
            (key: "A", value: 1),
            (key: "A", value: 2),
        ]
        ...
        XCTAssert(orderedDictionary._orderedKeys[0] == "A")
        XCTAssert(orderedDictionary._valuesForKeys.count == 1)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[0]] == 2)
    }
    
    func testInitWithArrayLiteral_whichContainsThreeElements() {
        let orderedDictionary: OrderedDictionary = [
            (key: "A", value: 1),
            (key: "B", value: 2),
            (key: "C", value: 3),
        ]
        XCTAssert(orderedDictionary._orderedKeys.count == 3)
        XCTAssert(orderedDictionary._orderedKeys[0] == "A")
        XCTAssert(orderedDictionary._orderedKeys[1] == "B")
        XCTAssert(orderedDictionary._orderedKeys[2] == "C")
        XCTAssert(orderedDictionary._valuesForKeys.count == 3)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[0]] == 1)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[1]] == 2)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[2]] == 3)
    }
    
    // MARK: Testing Initialization with Dictionary Literal
    func testInitWithDictionaryLiteral_whichContainsOneKeyValuePair() {
        let orderedDictionary: OrderedDictionary = ["A" : 1]
        XCTAssert(orderedDictionary._orderedKeys.count == 1)
        XCTAssert(orderedDictionary._orderedKeys[0] == "A")
        XCTAssert(orderedDictionary._valuesForKeys.count == 1)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[0]] == 1)
    ...
    func testInitWithDictionaryLiteral_whichContainsTwoKeyValuePair() {
        let orderedDictionary: OrderedDictionary = ["A" : 1, "B" : 2]
        XCTAssert(orderedDictionary._orderedKeys.count == 2)
        XCTAssert(orderedDictionary._orderedKeys[0] == "A")
        XCTAssert(orderedDictionary._orderedKeys[1] == "B")
        XCTAssert(orderedDictionary._valuesForKeys.count == 2)
    ...
    // MARK: Testing Initialization with Values and Key Provider Closure
    func testInitializationUsingValuesAndKeyProviderClosure() {
        let values = [1, 2, 3]
        let orderedDictionary = OrderedDictionary(values: values, keyedBy: { "\($0)" })
        XCTAssert(orderedDictionary._orderedKeys.count == 3)
        XCTAssert(orderedDictionary._orderedKeys[0] == "1")
        XCTAssert(orderedDictionary._orderedKeys[1] == "2")
        XCTAssert(orderedDictionary._orderedKeys[2] == "3")
        XCTAssert(orderedDictionary._valuesForKeys.count == 3)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[0]] == 1)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[1]] == 2)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[2]] == 3)
    ...
    func testInitializationUsingValuesAnyKeyPath() {
        ...
        let orderedDictionary = OrderedDictionary(values: values, keyedBy: \.string)
        ...
        XCTAssert(orderedDictionary._orderedKeys[2] == "C")
        XCTAssert(orderedDictionary._valuesForKeys.count == 3)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[0]] == values[0])
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[1]] == values[1])
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[2]] == values[2])
    }
    
    func testInitializationUsingDisorderedDictionaryAndSortFunction() {
        let unsorted = [
            2: "foo",
            1: "bar",
            4: "baz",
        ...
        let orderedDictionary = OrderedDictionary(unsorted: unsorted, areInIncreasingOrder: { $0.key < $1.key })
        
        XCTAssert(orderedDictionary._orderedKeys.count == 5)
        XCTAssert(orderedDictionary._orderedKeys[0] == 1)
        XCTAssert(orderedDictionary._orderedKeys[1] == 2)
        XCTAssert(orderedDictionary._orderedKeys[2] == 3)
        XCTAssert(orderedDictionary._orderedKeys[3] == 4)
        XCTAssert(orderedDictionary._orderedKeys[4] == 5)
        XCTAssert(orderedDictionary._valuesForKeys.count == 5)
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[0]] == unsorted[1])
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[1]] == unsorted[2])
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[2]] == unsorted[3])
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[3]] == unsorted[4])
        XCTAssert(orderedDictionary._valuesForKeys[orderedDictionary._orderedKeys[4]] == unsorted[5])
    }
    
    // MARK: Testing Content Accessing
    func testAccessingContent() {
        let orderedDictionary: OrderedDictionary = ["A": 1, "B": 2, "C": 3]
    ...
    func testAccessingDisorderedDictionary() {
        let orderedDictionary: OrderedDictionary = ["A": 1, "B": 2, "C": 3]
        
        let expected = ["A": 1, "B": 2, "C": 3]
        let actual = orderedDictionary.dictionary
    ...
    func testKeyBasedModifications() {
        var orderedDictionary: OrderedDictionary = ["A": 1, "B": 2, "C": 3]
    ...
    func testIndexBasedInsertionWithDuplicateKey() {
        ...
        XCTAssertFalse(orderedDictionary.canInsertElement(forKey: invalidKey))
    }
    ...
    func testIndexBasedInsertionWithNegativeIndex() {
        ...
        XCTAssertFalse(orderedDictionary.canInsertElement(at: invalidIndex))
    }
    ...
    // MARK: - Testing Removing Contents
    func testRemoveAll() {
        var orderedDictionary: OrderedDictionary = ["A": 1, "B": 2, "C": 3]
    ...
    func testIndexBasedRemoval() {
        var orderedDictionary: OrderedDictionary = ["A": 1, "B": 2, "C": 3, "D": 4]
    ...
    func testSortingWithMutation() {
        var orderedDictionary: OrderedDictionary = ["E": 4, "G": 3, "A": 3, "D": 1, "B": 4]
    ...
    func testSortingWithoutMutation() {
        let orderedDictionary: OrderedDictionary = ["E": 4, "G": 3, "A": 3, "D": 1, "B": 4]
    ...
    func testSliceAccess() {
        let orderedDictionary: OrderedDictionary = ["A": 1, "B": 2, "C": 3, "D": 4]
    ...
    struct _Dummy: Equatable {
        var string: String
        
        static func == (lhs: _Dummy, rhs: _Dummy) -> Bool {
